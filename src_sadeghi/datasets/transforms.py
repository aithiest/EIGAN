...
class Pad(object):
    ...
    def __call__(self, input):
        input['img'] = ImageOps.expand(
            input['img'], border=self.padding, fill=self.fill)
        return input
...
class RandomCrop(object):
    ...
    def __call__(self, input):
        if self.padding > 0:
            input['img'] = ImageOps.expand(
                input['img'], border=self.padding, fill=0)
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, input):
        if random.random() < 0.5:
            ...
            input['loc'][0] = input['loc'][0] - \
                math.ceil(input['img'].size[0] / 2)
        return input
...
class AffineCrop(object):
    ...
    def __call__(self, input):
        ...
        # perform scaling
        img = img.resize(
            (int(math.ceil(scale*h)), int(math.ceil(scale*w))), Image.ANTIALIAS)
        ...
        if (np.sum(input['loc']) != 0):
            ...
            trans_matrix = np.array(
                [[1, 0, -1*transx], [0, 1, -1*transy], [0, 0, 1]])
            scale_matrix = np.array([[scale, 0, 0], [0, scale, 0], [0, 0, 1]])
            angle_matrix = np.array([[math.cos(grad_angle), math.sin(
                grad_angle), 0], [-math.sin(grad_angle), math.cos(grad_angle), 0], [0, 0, 1]])
        ...
        for i in range(self.nlandmark):
            if (not np.isnan(input['loc'][:, i][0]) and not np.isnan(input['loc'][:, i][1])):
                tmp = self.utils.gaussian(
                    np.array([self.ix, self.iy]), input['loc'][:, i], self.gauss)
        ...
        tmp = self.utils.gaussian(
            np.array([self.iy, self.ix]), input['loc'][:, -1], 4 * self.gauss)
        scaled_tmp = sp.misc.imresize(tmp, [self.ox, self.oy])
        scaled_tmp = (scaled_tmp - min(scaled_tmp.flatten())) / \
            (max(scaled_tmp.flatten()) - min(scaled_tmp.flatten()))
        ...
        if self.keep_landmarks_visible:
            ...
            while not (mask == mask * input['occ']).all() or utils.isnan(landmarks[mask2D]).any():
                ...
                if limit == 0:
                    ...
                    _just_resize()
                    print(
                        'using the orignal data because even after 100 transformation, there are still occluded landmarks!!!')
                    break
...
class MakePartialBlockage(object):
    ...
    def __call__(self, sample):
        ...
        def _transform(newSample):
            ...
            loc_idx = 2
            block_pos_x = int(
                min(max(0, loc[0][occ][loc_idx]-block_size[1]/2), (img_size_w - block_size[1])))
            block_pos_y = int(
                min(max(0, loc[1][occ][loc_idx]-block_size[0]/2), (img_size_h - block_size[0])))
    ...
    def __call__(self, landmarks, input_resolution):
        ...
        for i in range(landmarks.shape[1]-1):
            if not np.isnan(landmarks[:, i]).any():
                ...
                scaled_tmp = sp.misc.imresize(
                    tmp, [self.outputH, self.outputW])
                scaled_tmp = (scaled_tmp - min(scaled_tmp.flatten())) / (
                    max(scaled_tmp.flatten()) - min(scaled_tmp.flatten()))
            else:
...
class ToColorHeatmap:
    ...
    def __call__(self, input, resolution=None):
        ...
        colored_heatmap[0] = self.gauss(
            input, 1, 1, 0.001) + self.gauss(input, 0.7, 1, 0.4)
    ...
    def color_heatmap(x):
        ...
        color[:, :, 0] = ToColorHeatmap.gauss(
            x, .5, .6, .2) + ToColorHeatmap.gauss(x, 1, .8, .3)
...
class ToLandmarks:
    ...
    def _get_preds(scores):
        ...
        maxval, idx = torch.max(scores.view(
            scores.size(0), scores.size(1), -1), 2)
