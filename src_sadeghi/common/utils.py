...
def eigan_loss(losses, weights, n_classes):
    assert len(losses) == len(weights)
    ans = 0
    for l, w, n in zip(losses, weights, n_classes):
        if w >= 0:
            ans += l*w
        else:
            ans += w*l
...
def to_categorical(y, num_classes=None, dtype='float32'):
    """
    Keras implementation to one-hot encode the target labels
    """
...
def laplaceMechanism(x, epsilon, device):
    """
    Adds 0-mean laplace noise to the input x where
    standard deviation is monitored by argument epsilon
    """
    x += torch.Tensor(np.random.laplace(0, 1.0/epsilon, x.shape)).to(device)
    return x

# https://github.com/eriklindernoren/PyTorch-YOLOv3/blob/47b7c912877ca69db35b8af3a38d6522681b3bb3/utils/parse_config.py
...
def horisontal_flip(images, targets_0, targets_1=None):
    images = torch.flip(images, [-1])
    targets_0[:, 2] = 1 - targets_0[:, 2]
#    targets_1[:, 2] = 1 - targets_1[:, 2]
    return images, targets_0, targets_1
